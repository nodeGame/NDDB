 ...
 * @see `streamingWrite`
 */
function loadCsv(that, filename, readCb, doneCb, options, method, errorCb) {
 ...
 *
 * @param {NDDB} that The NDDB instance
 * @param {object} opts Initial options (will be modified)
 * @param {string} method The name of the invoking method
 *
 * @see README.md
 * @see http://nodejs.org/api/fs.html#fs_fs_createwritestream_path_options
 */
function setDefaultCsvOptions(that, opts, method) {
    if (opts.columnNames) opts.columnNames = undefined;

    // Backward-compatible header.
    setOptHeader(opts);

    // Set default header.
    if ('undefined' === typeof opts.header && that.defaultCSVHeader) {
        opts.header = that.defaultCSVHeader;
    }

    checkHeaderAdd(that, opts, method);

    if (opts.flag) {
        console.log('***NDDB.' + method + ': option flag is deprecated ' +
                    'use flags***');
        if (!opts.flags) opts.flags = opts.flag;
    }

    if (method === 'load' || method === 'loadSync') {

        // Note: 'all' is not a valid option for load.
        if ('string' === typeof opts.header) {
    ...
    opts.escapeCharacter = opts.escapeCharacter ?? '\\';
    ...
    if ('undefined' !== typeof opts.objectLevel) {
        opts.scanObjects.level = opts.objectLevel;
    }
}
...
function processFileStreamInSync(fd, lineProcessorCb, options) {
    ...
    // Work done, exit here.
    if (line) {
        // If the last chunck of file does not end with the lineBreak,
