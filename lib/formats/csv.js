...
function setDefaultCsvOptions(that, options, method) {
    ...
    if (method === 'load' || method === 'loadSync') {
        // TODO: columnsFromHeader seems not to be used.
        if (!options.header &&
...
function getCsvHeaderAndFlatten(data, opts) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (opts.deepFlat) tmp = deepFlat(tmp);
 ...
 * @param {string} prefix Optional. The prefix to add before each key,
 *                        also used for recursion
 * @param {object} obj Optional. An initial object to contain for the output.
 *
 * @return {object} result The flattened object
 **/
function deepFlat(obj, prefix = false, result = null) {
    result = result || {};
    // Preserve empty objects and arrays, they are lost otherwise
    if (prefix && 'object' === typeof obj &&
    ...
    for (const i in obj) {
        if (obj.hasOwnProperty(i)) {
            if (typeof obj[i] === 'object' && obj[i] !== null) {
                ...
                deepFlat(obj[i], prefix + i, result);
            }
            else {
