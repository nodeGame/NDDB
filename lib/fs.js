...
(function() {
     ...
     *
     * @return {object} The files cache for either all files or a specific file
     */
    NDDB.prototype.getFilesCache = function(filename) {
        var cache;
        cache = this.__parentDb ? this.__parentDb.__cache : this.__cache;
        return filename ? cache[filename] : cache;
    };
    ...
    function saveCsv(that, filename, writeCb, doneCb, opts, method, errorCb) {
        ...
        var i, len;
        var flat;
        var firstSave, cache, recurrent, recurrentInterval;
        ...
        cache = that.getFilesCache();
        if (!cache[filename]) {
            cache[filename] = { lastSize: 0 };
        }
        else {
            firstSave = false;
