...
(function() {
    ...
    function saveCsv(that, filename, writeCb, doneCb, opts, method, errorCb) {
        ...
        var i, len;
        var flat;
        var firstSave, recCache, recurrent, recurrentInterval;
        ...
        if (recurrent) {
            ...
            if (!that.__recurrent[filename]) {
                that.__recurrent[filename] = { lastSize: 0 };
            }
            else {
                firstSave = false;
            }
            recCache = that.__recurrent[filename];

            // True only if user calls a recurrent save function again.
            if (!firstSave) {
                // Try to recover headers.
                if ('undefined' === typeof opts.headers) {
        ...
        // Store headers.
        if (recurrent) recCache.headers = headers;
        ...
        if (recurrent) {
            ...
            setInterval(function() {
                ...
                if (recCache.lastSize === data.length) return;
                // Store new size.
                // Note: currentItem in writeIt keeps track of where we left.
