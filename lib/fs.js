...
(function() {
    ...
    function getCsvHeaderAndFlatten(data, opts) {
        ...
        var group, groupsMap, doGroups, counter, flattenGroups;
        ...
        doGroups = !!opts.flattenGroups;
        if (doGroups) {
            // To dynamically modify the data array with the groups, we need
            // to keep track of the group position in the data array.
            groupsMap = {};
            // Option flattenGroups can be a function or a string (already
            // validated). If string, change it into a function getting the
            // property value (nested values accepted).
            if ('string' === typeof opts.flattenGroups) {
                flattenGroups = function(item) {
                    return J.getNestedValue(opts.flattenGroups, item);
                };
            }
            else {
                flattenGroups = opts.flattenGroups;
            }
        }
