...
(function() {
    ...
    NDDB.prototype.setWD = function(wd, force) {
        ...
        // Update the working directory if views and hashes.
        // Views.
    ...
    function saveCsv(that, filename, writeCb, doneCb, opts, method, errorCb) {
        ...
        var i, len;
        var flat;
        var recCache, firstSave, recurrent;
        ...
        if (recurrent) {
            if (!that.__recurrentData[filename]) {
                that.__recurrentData[filename] = { lastSize: 0 };
                firstSave = true;
            }
            recCache = that.__recurrentData[filename];

            if (!firstSave) {
                // Try to recover headers.
                headers = recCache.headers;
                if ('undefined' !== typeof headers) {
                    opts.headers = headers;
                    recCache.headers = headers;
                }

                // Unless otherwise specified, append.
                if ('undefined' === typeof opts.flag) opts.flag = 'a';
            }

            // TODO: Slice here?
            // if (opts.incremental !== false &&
            //     recCache.lastSize !== data.length) {
            //
            //     data = data.slice(recCache.lastSize-1);
            // }
            console.log('LEN: ' , data.length);
            console.log(opts);
            console.log('h len', headers ? headers.length : 0);
        }
        ...
        // Note: headers can be falsy, and if so all keys from items are used.

        // Store headers. TODO: maybe we should do it later.
        if (opts.recurrent && recCache) recCache.headers = headers;

        // Create callback for the writeCb function.
        writeIt = (function() {
            // Self calling function for closure private variables.
            var len, firstCall, currentItem;
        ...
        if (recurrent) {
            console.log('Setting up recurrent saving.');
            setInterval(function() {
                var newSize;

                console.log('Recurrent saving.');
                // Important, reuse variable.
                data = that.fetch();
                newSize = data.length;
                // Stop here if no changes in database.
                if (recCache.lastSize === newSize) return;
                console.log('Recurrent saving ahead.');

                // Get new items only.
                // if (opts.incremental !== false) {
                    //data = data.slice(recCache.lastSize-1);
                //}
                // currentItem keeps track of what saved.
                console.log('Data length: ', data.length);

                // Store new size.
                recCache.lastSize = newSize;
