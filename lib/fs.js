...
(function() {
    ...
    NDDB.prototype.addDefaultFormats = function() {
        ...
        this.__formats.json = {
            ...
            loadSync: function(that, file, cb, opts) {
                loadJSON(that, fs.readFileSync(addWD(that, file), opts));
                if (cb) cb();
            },
    ...
    NDDB.prototype.sync = function(opts = {}) {
        ...
        let conf =  {
            format: 'json',
            updateDelay: opts.updateDelay || 2000,
            header: opts.header || this.defaultCSVHeader
        };
        if (opts.headerAdd) conf.header = [ ...conf.header, ...conf.headerAdd ];
        ...
        this.on('insert', function(item) {
            ...
            saveTimeout = setTimeout(function() {
                ...
                let db = that.fetch();
                db = db.slice(lastSize)
                lastSize = newSize;

                // Save it.
                if (conf.format === 'json') {
                    NDDB.save(db, filename, {
                        append: true,
                        compress: true,
                        enclose: false
                    });
    ...
    function saveCsv(that, filename, writeCb, doneCb, opts, method, errorCb) {
        ...
        if (flat) {
            ...
            header = getCsvHeader(header, opts.scanObjects, data);
        }
    ...
    * ### setOptHeader
    *
    * Returns the header option, looks for both header and header
    *
    ...
    */
     ...
     *
     * @param {string|array|function} headerOpt Optional. Parameter
     ...
     * @see JSUS.keys
     */
    function getCsvHeader(headerOpt, objectOptions, data) {
        var h;
        h = null;
        if (headerOpt) {
