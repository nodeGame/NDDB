...
(function() {
    ...
    NDDB.load = function(filename, opts) {
        var nddb;
        nddb = new NDDB();
    ...
    NDDB.prototype.addDefaultFormats = function() {
        ...
        this.__formats.json = {
            ...
            save: function(that, file, cb, opts) {
                ...
                let str = opts.stream ? ',' : '';
                str += that.stringify(opts.compress, opts.enclose);

                fs.writeFile(addWD(that, file), str, opts, cb);
            },
            // Sync.
            loadSync: function(that, file, cb, opts) {
                loadJSON(that, fs.readFileSync(addWD(that, file), opts), opts);
                if (cb) cb();
            },
            loadSyncAll: function(that, dir, cb, opts) {
    ...
    NDDB.prototype.sync = (function() {
        ...
        // Execute with NDDB context.
        function syncCb(op, nddbid, item)  {
            ...
            saveTimeout = setTimeout(() => {
                ...
                // let journal = this.fetch();
                // journal = journal.slice(cache.lastSize)
        ...
        return function(opts) {
            ...
            filename = conf.filename || `${this.name}.${conf.format}`;
            ...
            if (conf.format === 'csv') {
                ...
                this.on('remove', function(item) {
                    syncCb.call(this, 'd', item._nddbid, null);
                });

                this.on('insert', function(item) {
                    syncCb.call(this, 'i', item._nddbid, item);
                });

                this.on('update', function(item, update) {
                    syncCb.call(this, 'u', item._nddbid, update);
                });
            }
        };
    ...
    */
    ...
    function getCsvHeader(data, opts) {
        var h, headerOpt, objectOptions, headerAdd;
        h = null;
    ...
    function loadJSON(nddb, s, opts) {
        ...
        if (opts.enclose) s = '[' + s + ']';
        if (opts.retrocyle !== false) cb = NDDB.retrocyle;
        if (opts.cb) {
            customCb = opts.cb;
            if (cb) {
                cb = function() {
    ...
    NDDB.prototype.importSync = function(items) {
        for (let i = 0; i < items.length; i++) {
            ...
            if (o.op === 'i') {
                ...
                this.throwErr('Error', 'importSync',
                              'unknown operation item. Found: ' + o.op);
            }
        }
    };
     ...
     * @see JSUS.keys
     */
    function getAllKeysForHeader(db, headerOpt, objectOptions, headerAdd) {
        var i, len;
        i = -1, len = db.length;
        processJSUSKeysOptions(objectOptions, headerOpt, headerAdd);
        for ( ; ++i < len ; ) {
    ...
    function getCsvHeaderAndFlatten(data, opts) {
        ...
        var headerOpt, objectOptions, headerAdd;
     ...
     * @api private
     */
    function processJSUSKeysOptions(objectOptions, headerOpt, headerAdd) {
     ...
     * @api private
     */
    const _doRecSearch = (dir, level, maxLevel, filter) => {
        let out = [];
        ...
        .forEach(file => {
            ...
            if (file.isDirectory()) {
                ...
                if (filter) {
                    ...
                    new RegExp(filter).test(file.name) : filter(file.name);
                }
                // console.log(res, level, maxLevel, filePath);
                if (res) out.push(filePath);
            }
        });
        return out;
    };
    ...
    const getFilesSync = (dir, opts = {}) => {
        ...
        return _doRecSearch(dir, 0, maxRecLevel, opts.filter);
    };
