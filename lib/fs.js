...
(function() {
    ...
    NDDB.prototype.addDefaultFormats = function() {
        ...
        this.__formats['csv'] = {
            ...
            load: function(that, file, cb, options) {
                ...
                setDefaultCsvOptions(options, 'load');

                reader = csv.createCsvFileReader(file, options);

                if (options.headers) {
                    reader.setColumnNames(options.headers);
                }
            ...
            save: function(that, file, cb, options) {
                ...
                var writer, headers, i, na, row, adapter;
                var data;
            ...
            // Sync.
            loadSync: function(that, file, cb, options) {
                var headers, separator, quote, escapechar;
                var obj, tokens, i, len;

                // Options.
                setDefaultCsvOptions(options, 'loadSync');
            ...
            saveSync: function(that, file, cb, options) {
                ...
                var separator, quote, escapechar, na;
                var data;
                var i, len;
    ...
    function setDefaultCsvOptions(options, method) {
        ...
        options.escapechar = options.escapechar || options.quote;
    }
        ...
        if (!headers) {
            for (key in item) {
                if (item.hasOwnProperty(key)) {
                    out += createCsvToken(item[key], quote, escapechar,
                                          na) + separator;
                }
            }
        }
            ...
            headers.forEach(function(h) {
                ...
                out += createCsvToken(str, quote, escapechar, na) +
                    separator;
            });
        }
        // Remove last comma.
     ...
     * @return {string} The parsed token
     */
    function createCsvToken(token, quote, escapechar, na) {
        var i, len, out, c;
