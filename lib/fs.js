...
(function() {
            ...
            return function(row) {
                ...
                for (j = 0, i = 0; j < tokens.length; ++j) {
                    ...
                    if (token.charAt(token.length-1) !== escapeCharacter) {
                        ...
                        str = result.split(escapeCharacter + quote).join(quote)
    ...
    function saveCsv(that, filename, writeCb, doneCb, options,
        ...
        data = that.fetch();
        headers = getCsvHeaders(options.headers, data, options.processHeaders);
        adapter = options.adapter || {};
     ...
     * @param {} data Optional. Array containing the items to save
     *
     * @return {array|null} h The headers, or NULL if not found
     */
    function getCsvHeaders(headers, data, cb) {
        var h, i, len, tmp;
        h = null;
        if (headers) {
            if (headers === 'all') h = collectAllHeaders(data);
            else if (J.isArray(headers)) h = headers;
        }
        ...
        if (h && cb) {
            console.log(h);
            i = -1, len = h.length;
            // Important! We need to keep tracks of the modifications
            // to the array length and apply to the indexes.
            for ( ; ++i < len ; ) {
                tmp = cb(h[i]);
                ...
                // If array, expand it.
                    ...
                    h.splice.apply(h, [i, 1].concat(tmp));
                    i+= tmp.length-1;
                    len += tmp.length-1;
                }
                // Else ignored.
     ...
     * ### collectHeaders
     *
     * Collects all the unique keys of all the
     *
     */
    function collectAllHeaders(db) {
        var i, len;
        var item, key, headers, out;
        headers = {}, out = [];
        i = -1, len = db.length;
        for ( ; ++i < len ; ) {
            item = db[i];
