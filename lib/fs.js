    ...
    this.__formats['csv'] = {
        ...
        saveSync: function(that, file, cb, options) {
            ...
            if (headers) {
                ...
                for ( ; ++i < len ; ) {
                    out += createCsvToken(headers[i], quoteChar, escapeChar);
 ...
 */
function getCsvRow(that, method, item, headers, adapter, separator, quoteChar,
                   escapeChar, na) {
    ...
    if (!headers) {
        for (key in item) {
            if (item.hasOwnProperty(key)) {
                out += createCsvToken(headers[i], quoteChar, escapeChar, na) +
                    separator;
            }
        }
    }
        ...
        headers.forEach(function(h) {
            ...
            out += createCsvToken(str, quoteChar, escapeChar, na) + separator;
        });
    }
    // Remove last comma.
