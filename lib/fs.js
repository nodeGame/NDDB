...
(function() {
    ...
    NDDB.prototype.addDefaultFormats = function() {
        ...
        this.__formats.csv = {
            ...
            save: function(that, file, cb, options) {
                var out;
                out = processCsvForWrite(that, 'save', options);
            ...
            saveSync: function(that, file, cb, options) {
                var out = processCsvForWrite(that, 'saveSync', options);
    ...
    function processCsvForWrite(that, method, options) {
        ...
        data.forEach(function(item) {
            out += getCsvRow(that, method, item, headers, adapter,
                             separator, quote, escapeCharacter, na) + '\n';
        });
        return out;
    }
     ...
     * ### streamingReadSync
     *
     * Invokes the functions fs.openSync and processFileStreamSync
     *
    ...
    function streamingReadSync(filename, lineProcessorCb, doneCb, options) {
        ...
        processFileStreamSync(fd, lineProcessorCb, options);
        if (doneCb) doneCb();
    }
     ...
     * ### streamingRead
     *
     * Invokes the functions fs.open and processFileStreamSync
     *
     * @param {string} filename. Name of file to load into database
     * @param {function} cb. Callback to be executed upon completion
     * @param {object} options. Initial options
    ...
    function processFileStreamSync(fd, lineProcessorCb, options) {
