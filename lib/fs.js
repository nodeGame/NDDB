...
(function() {
    ...
    function saveCsv(that, filename, writeCb, doneCb, opts, method, errorCb) {
        ...
        if (flat) {
            // data is overwritten with an array of size one.
            [ data, headers ] = flatten(data, opts);
        }
        else {
    ...
    function setDefaultCsvOptions(options, method) {
        ...
        // If options.flatten is true, options headers behave slightly
        // differently
        // if (options.flatten) {
        //
        ...
        if (!headers) {
            for (key in item) {
                if (item.hasOwnProperty(key)) {
                    tmp = preprocessItem(item, key, adapter);
            ...
            len = headers.length;
            for ( ; ++key < len ; ) {
                tmp = preprocessItem(item, headers[key], adapter);
    ...
    function preprocessItem(item, key, adapter) {
        var str;
    ...
    function collectAllHeaders(db, headersOpt, objectOptions) {
        var i, len;
        i = -1, len = db.length;
        processKeysOptions(objectOptions, headersOpt);
        for ( ; ++i < len ; ) {
    ...
    function processKeysOptions(objectOptions, headersOpt) {
