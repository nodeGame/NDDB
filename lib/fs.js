...
(function() {
    ...
    function saveCsv(that, filename, writeCb, doneCb, options,
        ...
        data = that.fetch();
        headers = getCsvHeaders(options.headers, options.objectLevel, data);
        adapter = options.adapter || {};
    ...
    function setDefaultCsvOptions(options, method) {
        ...
        if ('undefined' === typeof options.scanObjects) {
            options.scanObjects = {
                level: options.objectLevel || 0,
                concatKeys: options.concatObjectKeys || false,
                concatChar: options.concatObjectKeysChar || '.'
            };
        }


        // Check which syntax is best.
//         db.save('file.csv', {
//             scanObjectLevel:1,
//             scanObjectConcatKeys:1,
//             scanObjectConcatChar: 1
//         });
// 
//         db.save('file.csv', {
//             scanObjects: {
...
//         });
     ...
     * @param {number} objectLevel Optional In case headers must be extracted
     *   from data, it specifies the behavior of objects, how deep should
     *   nested keys be extracted. If the object has a deeper nested level
     *   than objectLevel, [Object object] will appear in the saved file.
     * @param {boolean} concatKeyNames Optional if objectLevel > 1, nested
     *   keys contains the parent object key name. E.g., `{ a: { b: 1 } }`
     *   will become a.b
     * @param {array} data Optional. Array containing the items to save
     *
     * @return {array|null} h The headers, or NULL if not found
     */
    function getCsvHeaders(headersOpt, objectLevel, concatKeyNames, data) {
        var h;
        h = null;
        if (headersOpt) {
            if (headersOpt === 'all' || 'function' === typeof headersOpt) {
                h = collectAllHeaders(data, headersOpt, objectLevel);
            }
            else if (J.isArray(headersOpt)) {
                h = headersOpt;
            }
            else if (headersOpt === true && data && J.isArray(data)) {
                h = J.keys(data[0], objectLevel, concatKeyNames);
    ...
    // allKeys, concatKeys, concatChar, res
    function getAllKeys(obj, level, curLevel, curParent,
                        concatKeys, allKeys, concatChar, res) {
        var key;
        res = res || [];
        ...
        for (key in obj) {
            if (obj.hasOwnProperty(key)) {
                if (allKeys || curLevel === level) {
                    if (concatKeys) res.push(curParent + key);
                    else res.push(key)
                }
                if ('object' === typeof obj[key] && curLevel < level) {
                    getAllKeys(obj[key], level, (curLevel+1),
                               concatKeys ? curParent + key + concatChar : key,
                               concatKeys, allKeys, concatChar, res);
                }
                else if (!allKeys && curLevel !== level) {                    
                    if (concatKeys) res.push(curParent + key);
                    else res.push(key)
                }
            }
     ...
     */
    function collectAllHeaders(db, headersOpt, objectlevel) {
        var i, len;
        var item, key, tmp, headers, out, cb, subKeys;
        ...
        for ( ; ++i < len ; ) {
            ...
            for (key in item) {
                if (item.hasOwnProperty(key)) {
                    ...
                    // Problems. Use stage.stage or just stage?
                    // J.keys returns stage + stage + step + round.
                    // Make a function for the inner part.
                    if (objectLevel > 0 && 'object' === typeof item[key]) {
                        subKeys = J.keys(item[key], objectLevel);
