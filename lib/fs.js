...
(function() {
    ...
    NDDB.prototype.addDefaultFormats = function() {
        ...
        this.__formats.csv = {
            ...
            save: function(that, file, cb, options) {
                var out;
                out = processCsvForWrite(that, file, 'save', options);
            ...
            saveSync: function(that, file, cb, options) {
                var out = processCsvForWrite(that, file, 'saveSync', options);
                ...
                for (j = 0; j < tokens.length; ++j) {
                    ...
                    if (token.charAt(token.length-1) !== escape) {
                        ...
                        processedTokens.push(parseCsvToken(
                            that, method, result, quote, escape
                        ));
                        result = "";
                    }
    ...
    function processCsvForWrite(that, file, method, options) {
        ...
        if (headers) {
            len = headers.length;
            for (i = 0 ; i < len ; ++i) {
                out += createCsvToken(headers[i], separator, quote,
                                      escape);
        ...
        data.forEach(function(item) {
            out += getCsvRow(that, method, item, headers, adapter,
        ...
        if (!headers) {
            for (key in item) {
                if (item.hasOwnProperty(key)) {
                    out += createCsvToken(item[key], separator, quote, escape,
                                          na) + separator;
                }
            }
        }
            ...
            headers.forEach(function(h) {
                ...
                out += createCsvToken(str, separator, quote, escape, na) +
                    separator;
            });
        }
        // Remove last comma.
     ...
     */
    function parseCsvToken(that, method, token, quote, escape) {
        ...
        // Replace all occurences without invoking RegExp constructor
            .split(escape + quote).join(quote)
            .split(escape + escape).join(escape)
            .replace(escape + 'n', '\n');
     ...
     * @return {string} The parsed token
     */
    function createCsvToken(token, separator, quote, escape, na) {
        var i, len, out, c;
        ...
        // Escape quotes, separator and escape char.
        if (quote) {
            out = quote;
            if (escape) {
                len = token.length;
                for (i = -1 ; ++i < len ; ) {
                    c = token.charAt(i);
                    if (c === quote || c === escape || c === separator) {
                        out += escape;
                    }
                    out += c;
                }
